...
3. **Spec Index** — Scan main repo's `specification/` for highest `[XX]` prefix. Next index = max + 1, zero-padded to 2 digits.
...
7. **Spec Directory** — `mkdir -p $WORKTREE_PATH/specification/[spec-identifier]`. Absolute path only.
...
9. **Workflow JSON** — Create `[spec-identifier]-workflow-tracking.json` from `${PLUGIN_ROOT}/reference/workflow-tracking-template.json`.
