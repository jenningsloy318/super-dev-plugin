...
<workflow>
  <!--
    This 14-stage contract is the source of truth for BOTH execution modes.
  ...
  <stage n="11" name="Integration Testing">OUTER LOOP (max 3): After code review PASSES, run integration tests. 11A (backend): api-tester generates+runs CRUD/validation/auth/edge tests. 11B (frontend): e2e-runner maps ALL BDD scenarios to Playwright, cross-browser, perf budgets, a11y. BLOCKING: 100% pass. On failure: fix → back to Stage 10 (re-review) → Stage 11 (re-test). Same error → ask user.</stage>
