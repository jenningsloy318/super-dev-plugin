...
- **Missing V8 (Behavior Coverage) checks when BDD scenarios exist**: When `[doc-index]-behavior-scenarios.md` is available, the reviewer must verify that every SCENARIO-XXX has a corresponding passing test. Skipping this check allows behavior gaps to slip through to production.
