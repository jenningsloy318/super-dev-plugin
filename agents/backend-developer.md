...
<collaboration>
  Runs as Step 9.2 in the sequential TDD workflow: tdd-guide (9.1) → backend-developer (9.2) → impl-summary-writer (9.3) → qa-agent (9.4). Receives test files from Step 9.1 and makes them pass.
</collaboration>
...
<mandatory-rules>
  ...
  <rule-file name="error-classification.md">Retriable vs terminal errors, backoff with jitter, error persistence, timeout budgeting, batch error isolation</rule-file>
  <rule-file name="enterprise-handler-patterns.md" condition="CRUD+enrichment endpoints">Upload-enrich-list pattern, async enrichment with SSE, rate-limited iteration, router wiring completeness, upsert with latest-wins</rule-file>
  <rule-file name="multi-file-sync.md" condition="RBAC, i18n, or schema changes">Coupled files that MUST be updated atomically — permission constants + roles + middleware + frontend guards</rule-file>
  <rule-file name="api-design-heuristics.md">Semantic type correctness, constructor delegation, reduce function arguments, extract deep nesting</rule-file>
  <rule-file name="config-design.md" condition="configuration schema changes">Nest under domain, condense enable+interval, per-entity tracking, human-readable defaults</rule-file>
</mandatory-rules>
