...
<process>
  <step n="0" name="Load Project-Specific Security Rules">For Rust network services, read `${plugin_root}/rules/rust-security-hardening.md` — check for: SecurityGate cascade (IP filter → rate limiter → connection tracker), RAII ConnectionPermit, lock-free token bucket, ArcSwap hot-reload, fuzz targets per crate, PDU validation at boundaries, deny.toml supply chain audit.</step>
