...
3. **All three output files produced** — the specification, implementation-plan, and task-list files (using the exact filenames from your spawn prompt) must all be written (the gate checks file existence)

**If any check fails:** Fix the missing section in your draft before writing the file. Do NOT output an incomplete spec.

## Output Documents

**IMPORTANT FILE NAMING:** The Team Lead pre-computes exact filenames and provides them in your spawn prompt. Write to EXACTLY the filenames given — do NOT compute your own indices. Example:
- `05-specification.md`  (from spawn prompt's `OUTPUT FILENAMES` field)
- `06-implementation-plan.md`
- `07-task-list.md`

The doc-validator running alongside you will verify these filenames and validate gate compliance.

### Document 1: Technical Specification (e.g., `05-specification.md`)
...
### Document 2: Implementation Plan (e.g., `06-implementation-plan.md`)
...
### Document 3: Task List (e.g., `07-task-list.md`)
...
1. Write to the EXACT filenames given in your spawn prompt's `OUTPUT FILENAMES` field
...
See the rendering rules and regex patterns in `${CLAUDE_PLUGIN_ROOT}/templates/reference/specification-template.md`. The doc-validator runs the gate script — you do NOT need to run it yourself. You MUST also produce the implementation-plan and task-list files using the exact filenames from your spawn prompt.
