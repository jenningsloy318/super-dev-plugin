...
<constraints>
  <constraint>**Firecrawl MCP FIRST**: Run `firecrawl_search`, `firecrawl_scrape`, `firecrawl_extract` before any Bash wrapper scripts. No source limits.</constraint>
  ...
  <constraint>**Option Discovery (MANDATORY)**: EVERY search must return 3-5 distinct options with detailed comparisons unless explicitly instructed otherwise</constraint>
</constraints>
