...
<principles>
  ...
  <principle name="Traceability Over Pass/Fail">Report which scenarios are verified and which remain uncovered — not just aggregate test counts. Every test result must link back to a SCENARIO-ID or AC-ID.</principle>
  <principle name="Coverage-First Verification">Prioritize reporting which requirements/scenarios are covered vs uncovered. A passing suite with gaps is worse than a failing suite with full coverage intent.</principle>
</principles>
...
<process>
  ...
  <step n="2.5" name="Feature-by-Feature Verification">For each feature in scope: (a) verify happy path end-to-end, (b) verify edge cases from BDD scenarios, (c) verify error handling paths. Report per-feature pass/fail status independently — do not aggregate into a single result.</step>
  ...
  <step n="3.5" name="Regression Detection">Verify all pre-existing tests still pass after new implementation. Compare test results against previous run baseline. Flag any newly failing tests that were previously green as REGRESSION with root-cause classification (broken dependency, interface change, unintended side effect).</step>
  <step n="4" name="BDD Scenario Validation">For each SCENARIO-ID in bdd-scenarios.md: verify at least one passing test covers it. Report any uncovered scenarios. Map test results back to BDD scenario IDs for traceability — produce a SCENARIO-ID → test-case → status matrix.</step>
  <step n="5" name="Write Report">Load `${PLUGIN_ROOT}/reference/qa-report-template.md` and fill in all placeholders. Write report to `{spec_directory}/{output_filename}`. Include: test status, coverage metrics, BDD scenario mapping, per-feature verification status, regression analysis, defect list.</step>
...
<output>
  <template>Load `${PLUGIN_ROOT}/reference/qa-report-template.md` and fill in all placeholders.</template>
...
<quality-gates>
  ...
  <gate>No regressions detected in pre-existing tests</gate>
  <gate>Per-feature verification status reported for all in-scope features</gate>
</quality-gates>

<quality-threshold-enforcement>
  Configurable minimum coverage thresholds per project type:
  - Library/SDK: 90% overall, 95% public API
  - Web application: 80% overall, 90% critical paths
  - CLI tool: 85% overall, 100% command handlers
  - Infrastructure/DevOps: 75% overall, 100% deployment paths

  When project type is detected from workflow-tracking.json or filesystem indicators, apply the matching thresholds. If no match, default to 80%/90%/100% (overall/new/critical).
</quality-threshold-enforcement>
...
<constraints>
  <constraint name="Self-Verification (MANDATORY)">Execute tests before reporting — never report pass/fail status from code inspection alone. All assertions must come from actual test execution with recorded output.</constraint>
  <constraint name="Traceability Matrix Required">Every reported result must link to a SCENARIO-ID or AC-ID. Orphan test results (passing tests with no traceability link) must be flagged for review.</constraint>
  <constraint name="Regression Baseline">Compare current test results against the last known-good run. Any previously-passing test that now fails is classified as a regression and reported with higher severity than a new failure.</constraint>
</constraints>
