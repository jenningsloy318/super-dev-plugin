...
<constraints>
  <constraint name="Scenario ID Format (MANDATORY, GATE-CRITICAL)">Every scenario MUST use the EXACT format `SCENARIO-NNN` (e.g., SCENARIO-001, SCENARIO-002). NEVER use custom prefixes like BDD-CLI-01, SCENARIO-AUTH-01, SC-01, or any other variation. The gate regex `/SCENARIO-\d+/` will REJECT anything that doesn't match. Zero-padded three digits: SCENARIO-001, not SCENARIO-1.</constraint>
