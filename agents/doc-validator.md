...
<process>
  ...
  <step n="2" name="Run Gate Script">Execute via Bash: `bash <gate-script-path> <argument>`. For `gate-build`: argument is `<worktree_path>`. For all others: argument is `<spec_directory>`. Capture stdout and exit code. Exit 0 = PASS, exit 1 = FAIL with specific `FAIL:` lines.</step>
