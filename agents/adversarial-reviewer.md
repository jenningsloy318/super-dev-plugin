...
<process name="Iteration Behavior">
  PASS → proceed to Stage 11 (Documentation). REJECT → MUST loop back to Stage 9 with findings as input for domain specialist to fix. There is no middle-ground verdict — all findings must be resolved before PASS.
</process>
