...
<collaboration>
  A `doc-validator` agent runs alongside during Phase 2. Respond to `VALIDATION FAILED` by fixing and replying `FIXED: ready for re-check`. Only report completion after `VALIDATED: PASS`.
</collaboration>
