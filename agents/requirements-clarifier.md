...
<process>
  <step n="0" name="Read Format Template">Read `{plugin_root}/templates/requirements.md.j2` to understand the expected markdown structure. This template shows the exact headings, sections, and formatting your output must follow.</step>
  ...
  <step n="3" name="Write Output Document">Write the requirements document directly to `{spec_directory}/{output_filename}` following the exact structure from the template you read in Step 0. The document MUST contain: Executive Summary, acceptance criteria with AC-XX IDs (format: `- **AC-01**: description`), Non-Functional Requirements section, and be longer than 500 characters.</step>
</process>
...
<output>
  <format>Write the requirements markdown document directly to `{spec_directory}/{output_filename}` following the structure from the template read in Step 0.</format>
...
<gate-format-requirements>
  Read `{plugin_root}/templates/requirements.md.j2` in Step 0 to understand the required markdown structure. The template shows exact headings, section order, and placeholder patterns the gate script validates. Write your output following that structure directly — no JSON or render.sh needed.
</gate-format-requirements>
