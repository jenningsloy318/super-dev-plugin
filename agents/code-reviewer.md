...
During Phase 9 you run alongside `adversarial-reviewer` and `doc-validator`. Use Direct Peer Communication signals (see SKILL.md):
...
- **adversarial-reviewer:** Share significant findings via `FINDING_SHARE`, acknowledge theirs via `FINDING_ACK`. Send `REVIEW_COMPLETE` when your verdict is written.
