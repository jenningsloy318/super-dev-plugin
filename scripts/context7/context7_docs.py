#!/usr/bin/env python3
"""
Context7 Docs Script: Get library documentation from Context7.
...
Usage:
    python3 context7_docs.py --library-id "/vercel/next.js" [--mode code|info] [--topic "routing"] [--page 1]
...
def main():
    parser = argparse.ArgumentParser(
        description=f"Get library documentation from Context7"
    )

    parser.add_argument(
        "--library-id", "-l",
        required=True,
        help="Context7-compatible library ID (e.g., '/vercel/next.js', '/mongodb/docs'). Use context7_resolve.py to find IDs."
    )
    parser.add_argument(
        "--mode", "-m",
        choices=["code", "info"],
    ...
    parser.add_argument(
        "--topic", "-t",
        help="Optional topic to focus documentation on (e.g., 'hooks', 'routing')"
    )
    parser.add_argument(
        "--page", "-p",
        type=int,
        default=1,
        help="Page number for pagination (1-10, default: 1)"
    )
    ...
    if args.topic:
        tool_arguments["topic"] = args.topic
