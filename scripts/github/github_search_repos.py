#!/usr/bin/env python3
"""
GitHub Search Repositories Script: Search for repositories by name, description, or metadata.

Connects to the GitHub MCP HTTP server configured in Claude Code.
Finds repositories across GitHub using advanced search syntax.

Usage:
    python3 github_search_repos.py --query "machine learning" [--sort stars] [--order desc] [--page 1] [--per-page 30]
...
def main():
    parser = argparse.ArgumentParser(
        description=f"Search GitHub repositories by name, description, or metadata",
        epilog="Examples:\n"
               "  Search by topic: --query 'topic:react'\n"
               "  Search by user: --query 'user:facebook'\n"
               "  Search with filters: --query 'machine learning stars:>1000 language:python'\n",
    ...
    parser.add_argument(
        "--query", "-q",
        required=True,
        help="Repository search query. "
             "Examples: 'machine learning', 'topic:react', 'user:facebook stars:>1000'"
    )
    parser.add_argument(
        "--sort", "-s",
        choices=["stars", "forks", "help-wanted-issues", "updated"],
        help="Sort repositories by field (defaults to best match)"
    )
    parser.add_argument(
        "--order", "-o",
        choices=["asc", "desc"],
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Return minimal repository information (default: true)"
    )
